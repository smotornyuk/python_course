...
последнего занятия с базами данных? Думаю, после него, сегодняшнее
занятия покажется достаточно несложным. А всё потому, что сегодня мы в
основном сконцентрировали свое внимание на пользовательском интерфейсе
и сведем к минимуму объем новой информации. Вместо этого мы будем
связывать воедино звенья цепочки разобранные на последних трех
...
И посему начнём мы сегодня с того, что выведем 6 десятков статей,
...
работы приложения не вносил. Так что вы можете заглянуть в предыдущие
...
Теперь, когда мы видим хоть какую-то информацию, можно добавлять и
действия. Возможно мы захотим добавить возможность создавать новые
статьи прямо на странице. Для этого нам нужна форма, то есть группа
полей в которые можно вести необходимую информацию нажать кнопочку
...
больше напоминает программирование чем верстку. Для нас, форма будет и
разделяться на две группы. Первая группа - это форма предназначенная
для получения информации, а вторая группа - это форма предназначена
для модификации данных, причём под модификация подразумевается как
обновление статей, так и создание, и даже удаление.
 ...
 отобразить данные. Дабы сильно не увлекаться, обращение браузера к
 серверу называют запросом, а отдача контента сервером браузеру —
 называют ответом. И запрос, и ответ имеют одинаковую структуру — они
 ...
 заголовки могут содержать информацию о том, на каком языке этот ответ
 ...
 позволяем посетителям не задумываться над всеми этими техническими
 деталями и просто вводить данные в понятный интерфейс. После чего
 форма сама уже их необходимым образом преобразует и отправит на
...
переделаем форму поиска на форму создания новой статьи. Так как это
...
авторизированный пользователь. Но раз у нас таковых нет, то автором
буду считаться Я, в абсолютно любой статье. Что касается даты
создания, то там проще всего — она у нас создается автоматически в
момент сохранения статьи, поэтому там заморачиваться даже не
нужно. Важно отметить, что по умолчанию, все поля вашей модели
являются обязательными, Поэтому если вы забудете их указать, то
сохранить элемент в базу у вас не выйдет. Если вы проверите это
утверждение, то вы заметите что я соврал. Ну, почти соврал. Тут дело в
том что по умолчанию Django использует SQ lite как систему управления
базами данных, и она не особо придирчиво относится к сохранению
информации. Но если вдруг вы переключитесь на что-то другое, а в
реальной жизни именно так и произойдет, то там у вас уже не получится
сохранить модель с пустыми полями. Так что сразу берём за правило — не
оставлять пустых значений.
...
форм. Хорошая новость - это уже предусмотрено в Django. Плохая новость
- вам придётся разобраться с ещё одним классом и его правилами
создания. Но, одним классом больше, одним меньше — вам уже всё-равно
нечего терять. Потому — начинаем всё автоматизировать направо и налево
...
отмечу что удаление статьи будут производиться с помощью
формы. Несмотря на то что кнопка удалить будет выглядеть вполне
самостоятельно, сам процесс удаления является модификацией данных, а
значит мы должны совершить его с помощью Post запроса. И следствие из
всего этого — кнопка удалить должна находиться внутри формы,
отправляющейся соответствующим методом. Если бы мы попытались здесь
...
несколько строк и оно начинает работать. Но у вас будут возникать не
только простые задачи поэтому сейчас мы попытаемся сделать нечто,
более сложное, а именно — обновить статьи. Для этого нам нужна будет
форма, в которой в момент загрузки страницы будут отображаться текущие
данные из статьи. Если мы эти данные исправим и нажмем сохранить,
статья должна обновиться в базе. На первый взгляд совсем ничего
сложного, но чтобы избежать большого объёма ненужного кода, нам
придётся добавить в нашу копилку еще один полезный класс.
...
Да хоть нам и пришлось немного попотеть прописывая весь этот код, его
однозначно меньше, чем было бы в реализации написанной без
использования Django. Но мы не делали ничего сверхъестественного, даже
более того — все задачи которые мы выполняли - это повседневная
рутина, и тратить на них несколько часов на каждом новом проекте не
хочется никому. Потому существует набор классов для реализации вот
таких вот стандартных задач. Используя эти классы вам нужно во-первых
...
правку любого контента. Возможно вы захотите добавлять пользователей в
...
у вас будут аккаунты, которые необходимо активировать. Как бы там ни
было, административная секция очень важнa, но нам не придется писать
ее с нуля, так как она уже предоставлена встроенными плагинами
Django. Давайте рассмотрим ее подробней
...
похожим на реальную жизнь, и можно считать что вы полноценные веб
...
до определённого момента, но мы вполне сумеем уложиться в бесплатную
...
веб-сайты. Разумеется, возможности выбора языка и технологии которые
тут используют не ограничивается лишь изученным. Более того, я
...
доставляет вам удовольствие и, разумеется, приносит деньги. Django
вполне подойдёт на эту роль. Тем более в нём есть так много готовых
плагинов, которые предоставляют кучу интересных фишек, так что вам
вряд ли станет скучно на нём работать ближайшие несколько лет.

И теперь вам перейдёт финальная и, так полагаю, самая сложная
контрольная. Выложитесь на полную, потому что она отражает суть всего
что мы проходили на этом курсе, ради неё мы с вами встречались на всех
этих занятиях и выполнение этой контрольной докажет что вы вполне
способны заниматься веб разработкой. Возможно даже вы захотите
использовать результат этой контрольной в своем портфолио. Ну а я на
этом с вами прощаюсь и надеюсь что когда-нибудь мы встретимся в
реальной жизни. Удачи, коллеги!