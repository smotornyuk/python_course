...
казаться что в коде скрыта какая-то магия, но мы то знаем что магии не
...
https://drive.google.com/open?id=1xFzM9phyTh1LWVrSg8sW2_ZyV7wITEX8 Модели

----------------------------------------------------------------

(Говорящая голова)

А сейчас у вас должно возникать ощущение того что вам приходится
делать очень много работы. Выходит, мы определяем какой-то класс, в
котором полностью описываем нашу таблицу, а потом ещё это таблицу
нужное создать? Вот тут вы не правы. Вам нужно только определить
класс, всё остальное за вас сделает Django.  Задача фреймворка,
обеспечить максимальную совместимость и воспроизводимость кода на
разных компьютерах. Потому, если каждый разработчик будет вручную у
себя создавать все необходимые таблицы в базе данных, то наверняка
кто-то где-то допустит ошибку. Дабы это избежать, вы описываете только
модель. Все остальные служебные действия по созданию таблиц,
обновлению колонок и так далее берёт на себя используемый вами
...
вы только начали работать над проектом никаких таблиц ещё не
...
присоединяется новый разработчик, у него есть описание всех миграций и
он может последовательно произвести все необходимые манипуляции с
...
и вы сможете откатиться на более ранние версии в случае если что-то
пойдет не так.

(видео конспект)


https://drive.google.com/open?id=1PH31O_g5iW8FHpqxGcbwBB0agEMvxWN5 Миграции
...
работе. Как уже говорилось не раз, никаких SQL команд вы здесь не
...
можем начинать работать с данными. Пока что, весь процесс
...
симпатичного юзер-интерфейса.

(видео конспект)

https://drive.google.com/open?id=1JMQMg2Ww2aSQy0p-VsW6dfA2V9sKlUpj Сохранение объектов
...
сильно влюбились в этот процесс нам не мешало бы иметь какие-то
...
контентом и автором. Я надеюсь, что вы настолько же ленивы насколько и
я, и не захотите вручную всё это печатать. Поэтому я расскажу вам про
одну небольшую, но очень удобную библиотеку для генерации в случайных
данных по заданным требованиям. И после того как у нас будет пара
десятков записей, мы сможем начать искать их, фильтровать и делать
кучу интересных вещей

(видео конспект)

https://drive.google.com/open?id=1m-_3NcIW0p4LAi4-dEEVBTqyilHBbuzq Генерация тестовых данных

----------------------------------------------------------------

(Говорящая голова)

Вот теперь, имея больше 60 записей в базе, мы можем перейти к самому
...
один из методов - подсчет записей - мы уже использовали. Если не
учитывать длину строки кода — ничего сложного там не было. Тут нужно
понять только общую идею организации процесса. Если мы хотим создать,
обновить или удалить конкретную статью, которая у нас уже присутствует
в виде объекта в переменной — то мы будем работать именно с этим
объектом. Но если у нас еще нет искомой статьи и нам ee нужно достать
из базы данных — то мы пока не можем работать с конечной переменной,
потому что ее просто нет и нам нужно выбрать некую точку входа, место
контакта с SQL. Для этого мы будем использовать еще одно из скрытых
свойств нашего класса статьи, которое символизирует все записи в
...
https://drive.google.com/open?id=103nBK-a4_kndJR-V0Nj6pptueJqUwpQ7 all-slice

----------------------------------------------------------------

(Говорящая голова)

Следующий вопрос который возникает - как получить не всю информацию,
...
https://drive.google.com/open?id=1BWOIny1Vkm_UkOmBLVT_ZWT1I7wKfjgB distinct-values-order

----------------------------------------------------------------

(Говорящая голова)

Ну и самая главная часть, которую я откладывал напоследок. Просто
...
вам понадобится извлечь только записи в которых имя автора равно
чему-то определенному. Либо все записи, чей рейтинг находятся в
диапазоне от и до. Либо все записи созданные определенного числа. В
общем, вам определённо нужно научиться фильтровать данные. Фильтрация
в Django имеет две формы — вы можете фильтровать, оставляя только те
записи, которые удовлетворяют некому условию. Либо вы можете
исключать, удаляя записи которые соответствуют условию. В целом,
результат не изменится, но исторически сложилось так, что присутствуют
две формы поиска. Они вполне совместимы и взаимозаменяемы, так что я
обычно предпочитаю первую форму

(видео конспект)


https://drive.google.com/open?id=14Ji2IcC054MLdGyj4u_xfgI837FmYSMS Фильтрация
...
статей с рейтингом больше чем некая граница или же поиск всех статей с
одним из допустимых авторов. Если кратко — поиск по нечеткому
...
https://drive.google.com/open?id=1DESTHVXUF5SKSvqGWNVI94Zx39TSAVaI Операторы фильтрации
...
также связана с фильтрацией, и, в принципе, нужна в той ситуации,
когда синтаксис питона не позволяет разгуляться. Сейчас мы рассмотрим
две дополнительные возможности. Первая из них, это поиск по нескольким
критериям в разных комбинациях. Временами вы можете захотеть, чтобы
все критерии поиска выполнялись, временами вам будет достаточно
выполнение лишь одного условия. И вторая возможность - это
...
следующий момент, вы знаете только то, что оно должна возрасти на
...
https://drive.google.com/open?id=1YEidLqMwAADdd_peth_2y5R0dSETefAk q-f
...
этому моменту вы уже должны, как минимум, быть согласны с тем что они
крайний прикольные. К ним быстро привыкаешь, использовать их гораздо
проще чем SQL, хотя бы по двум причинам. Первая - вам не нужно
беспокоиться о тонкостях, синтаксисе, стандартах которые присутствуют
в SQL. Вам даже не нужно заморачиваться используемой системой
...
потому код на нём должен был оказаться гораздо проще и очевиднее. Если
...
Рассказав всё это, я могу с чистой совестью идти отдыхать и готовить
