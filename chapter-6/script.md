...
стремился выделить всё на две противоположные категории. Например,
...
нельзя. В зависимости от типа данных однородность или обязательно или
нет. В случае строк — однородность избежать нельзя . Строка - это
коллекция символов , то есть Каждый элемент строки Это символ и значит
строка однородна. В списках же у вас нет каких-то конкретных
требований к типу содержимого.  Вы можете создать список чисел, строк,
других списков , но я простил вас стараться создавать однородные
списки. Это всего лишь моё пожелание, а не какое-то обязательное
требование языка . Так что списки — неоднородны.

 А вот изменяемости всё обстоит несколько сложнее — она также является
 некой абстракцией с различными толкованиями, но имеет гораздо большее
 влияние на то как вы пишите код и на возможности отдельных типов
 данных. Если попытаться сделать всё очевидным то можно сказать что
 изменяемые типы данных поддерживает модификация , а неизменяемые типы
 данных постоянны и не могут быть модифицированы.  А вот что же это
 значит на практике мы сейчас попробуем увидеть
...
становится коллекцией символ. Но мы увидели и различия - списки
изменяемы, благодаря чему У них есть некоторые особенности поведения ,
должен сказать, весьма Неожиданные.  Также в списке могут содержать
...
Прекрасно . И снова вернемся к изменяемости. выходит, имея на руках
 некий список A который вы ради эксперимента ещё присвоили и в
 переменную B, на деле у вас единственный список.  Изменение одной
 ...
 название . Во-первых, это яблоко друга . Во-вторых, Это ваши
 яблоко. Теперь становится понятным, Почему друг поедает свое яблоко,
 поедает ещё и ваши. Чтобы проблемы решить нам нужно умудриться
 клонировать наше яблоко и получить два самостоятельных и независимых
 экземпляра, которые только выглядят одинаково.  Пожалуй стоит
 переключиться обратно на списке и попробовать сделать это в
 интерпретаторе
...
обратно, чтобы всё выглядела не хуже — тут уже нужен определенный
навык. Во-первых мы можем использовать строчные методы, а именно join.
Он позволит нам объединить кусочки строки с помощью никого клея.
