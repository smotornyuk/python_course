...
коллекциях порядок элементов играет ключевую роль . в списке также
относится к порядочным коллекциям. Затем, коллекции было от
изменяемыми и неизменяемыми. Строки были неизменяемыми , благодаря
чему мы не могли изменить отдельную букву В существующей строки — для
...
хорошо индексов строках, вы без труда сумеете применить их и тут.  Как
мы сейчас увидим мы можем извлекать элементы из списка с помощью
индексов, мы можем использовать и срезы.  Аналогично строгом мы будем
...
из списков встречается больше элемент, то и весь список считается
...
бонусная элементы. В конце концов, если в 2 списках имеются одинаковые
элементы на одинаковых позиций и длина обоих списков тоже одинаково,
...
преобразования одного в другое. Трубку можно разбить на множество слов
— мы увидели это на прошлом занятии. Используя метод сплит мы
...
Обязательно должно существовать и обратная операция — Ничто Что
...
так много всего. Если быть честным то вы можете затолкать в список
...
отправить другу по почте — Попробуйте придумать на скидку 20 других
...
условия результирующее в правду либо ложь, После чего мы ставим
...
повторить. Но есть ли это самое первое или второе яблока, то рано ещё
...
урок. Если всё ещё непонятно — пересмотреть урок. Итак, возможно, до
бесконечности. Если в какой-то момент всё стало очевидным — прекратить
пересматривать и перейти к следующему уроку. Что нет никакой разницы
между реальной жизнью и программированием.
...
впервые есть не столкнулся. А со временем я узнал, что проблема
...
нечто подобное одному из существующих хорошее решений, а именно
...
строку на соответствие какому шаблону , изменяя регистр , сравнивает 2
...
как устроена работа с буквами внутри интерпретатора поэтому, ответ
...
вычитают. И вот как это можно использовать нам...
...
вот этом преобразование в числa.  Самое смешное, даже настоящeе
...
будущем , что каждый год 48 нужно воспринимать как цифру 1. запутано ,
но только если вы человек в машине же достаточно легко понять эту
...
на свежем воздухе. Некоторые темы нужно понимать не пристально глядя в
глаза а просто повторно перепроходя их спустя некоторое время. Не
