...
которые ещё и делают что-то полезное, но всё пройденное ранее в
большей степени касалось основ написания кода на питоне. Теперь же
пришло время поговорить о том, без чего не обойдется ни одна настоящая
...
разделить вашу программу на несколько файлов. Большую часть времени мы
уделим классам — потому что в питоне на них основывается абсолютно
всё. И, к тому же, они как раз таки и является тем инструментом с
помощью которого вы пишите объектно-ориентированный код.  Наверняка вы
...
что-то принципиально новое чего еще не существует в питоне, так и
создать слегка модифицированный и улучшенный тип данных на основании
уже существующего. Например, почему бы не создать умную строку , в
которой можно будет изменять отдельный символ, как элементы в
списках. Или же создать тип чисел , который позволяет деление на
ноль. И вот как обычно происходит описание нового типа данных
...
этого я вас серьёзно обманывал, говоря о том что методы содержатся в
определённых значениях. В действительности, ни одна строка не имеет
методов, все методы принадлежат классу строк. Сами же строки, эти
методы временами заимствует. То же самое касается и всех остальных
...
Хорошо, Вы уже должны были поверить в классы и убедиться в том что они
...
приходится писать функции, с учетом 2 возможных способов вызова. Это
...
все методы работают относительно некого объекта-жертвы. А если
...
мы захотим изменить собственное имя конкретно этого объекта. То
...
. Хотелось бы инициализировать объект стартовыми данными в более
простой форме. Во-вторых, мои объекты достаточно сложно выводить на
...
добавление новых свойств в объекте. Кроме того что вы можете
определить некий набор свойств в момент описания классов, на
...
отсутствовать. Например, мы можем попытаться считать свойство, которое
в этом объекте еще не возникло , что приведет нас к
...
перед его извлечением, хотя бы для отдельных свойств. А ещё лучше,
...
объектно-ориентированному программированию. Тема эта достаточно
обширная и сложна из-за обилия абстракций. Ну это судьба любого
инструмента — чем более полезным он остановится , чем больше в нём
...
абсолютно любого предмета, который вас окружаeт. Так что вы больше не
